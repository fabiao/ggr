...
demon::demon(const char *filename, int sx, int sy, BITMAP *bmp, int i, demon *p) : sprite(filename,bmp,sx,sy,i,p)
 {
  ...
  resetObj(true);
  ti[0]=new timer_D(1); // Walk time
  ti[1]=new timer_D(7); // Anim time
...
void demon::loadSample(const char *filename, int n)
 {
