...
insect::insect(const char *filename, int sx, int sy, BITMAP *bmp, int i, insect *p) : sprite(filename,bmp,sx,sy,i,p)
 {
  dd=-1; type=0;
  resetObj(true);
  ti[0]=new timer_D(2); // Anim time
  ti[1]=new timer_D(1); // Explode time
  ti[2]=new timer_D(random(500)+50); // Restart time
  ti[3]=new timer_D(0);
  ti[4]=new timer_D(0);
 }
...
void insect::loadSample(const char *filename, int n)
 {
