...
pork::pork(const char *filename, int sx, int sy, BITMAP *bmp, int i, pork *p) : sprite(filename,bmp,sx,sy,i,p)
 {
  ...
  resetObj(true);
  ti[0]=new timer_D(6); // Anim time
  ti[1]=new timer_D(4); // Jump up gravity time
  ti[2]=new timer_D(3); // Explode time
  ti[3]=new timer_D(10); // Shoot time
  ti[4]=new timer_D(100); // Stop time
 }
...
void pork::loadSample(const char *filename, int n)
 {
