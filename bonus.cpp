...
bonus::bonus(const char *filename, int sx, int sy, BITMAP *out, int fr, int i, bonus *p)
 {
  _i=i;
  ...
  ti[1]=new timer_D(150); // Bonus taked time
  ti[2]=new timer_D(0);
  ti[3]=new timer_D(0);
  ti[4]=new timer_D(0);
 }
...
void bonus::loadSpriteFrames(const char *filename)
 {
  BITMAP *aux;
...
void bonus::loadSample(const char *filename, int n)
 {
