...
weapon::weapon(const char *filename, BITMAP *out, int i, weapon *p)
 {
 	...
 	plat_wall=0;
 	ti[0]=new timer_D(1); // Hard time
 	ti[1]=new timer_D(1); // Anim time
 	ti[2]=new timer_D(0); // Gravity time
 	ti[3]=new timer_D(0);
 	ti[4]=new timer_D(0);
 }
...
void weapon::loadSpriteFrames(const char *filename)
 {
 	BITMAP *aux;
...
void weapon::loadSample(const char *filename, int n)
 {
	   	...
	   	if(pow)
	   	 {
	   	 	...
	   	 	if(index%multi==0)
	   	 	...
	   	 	else
	   	 	 {
	   	 	  grmax_life=56;
	   	 	  //frame_offset=99; min_frame=100; max_frame=101;
	   	 	  frame_offset=min_frame=max_frame=0; // Non buono per le collisioni poich� il frame 0 � vuoto
	   	 	  grframe_min=102; grframe_max=120;
...
void weapon::hitGround(int plat_y)
 {
 	// L'arma ha colpito il terreno, cambier� stato ed animazione
