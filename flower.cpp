...
flower::flower(const char *filename, int sx, int sy, BITMAP *bmp, int i, flower *p) : sprite(filename,bmp,sx,sy,i,p)
 {
  min_x=max_x=0;
  ...
  resetObj(true);
  ti[0]=new timer_D(5);   // Anim time
  ti[1]=new timer_D(400+random(200)); // Restart time
  ti[2]=new timer_D(200); // Shoot time
  ti[3]=new timer_D(10);  // Shoot anim time
  ti[4]=new timer_D(0);
 }
...
void flower::loadSample(const char *filename, int n)
 {
