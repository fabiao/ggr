...
particle::particle(const char *filename, int sx, int sy, BITMAP *out, int i, particle *p)
 {
  _i=i;
  ...
  ti[1]=new timer_D(0); // Up time
  ti[2]=new timer_D(0); // Down time
  ti[3]=new timer_D(0); // Left time
  ti[4]=new timer_D(0); // Right time
 }
...
void particle::loadSFXFrames(const char *filename)
 {
  BITMAP *aux;
...
void particle::loadSample(const char *filename, int n)
 {
