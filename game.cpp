...
void Game::init()
 {
  ...
  // Set the screen
  // Fullscreen patch for MacOS (Thanks to J�r�me Rabas)
...
void Game::initGame(int st,int li, int di)
 {
  // New game start
...
void Game::restartStage(int stage)
 {
  int dress;
...
void Game::readConf(int *screenres, int *scanlines, bool *fscreen, bool *stretch,bool *debug,int *keyleft,int *keyright,int *keyup,int *keydown,int *keyjump,int *keyshoot,int *music_volume,int *sound_volume)
 {
  FILE *fd;
  char dummy[50];
  int aux;
...
void Game::writeConf(int screenres, int scanlines, bool fscreen, bool stretch, bool debug,int keyleft,int keyright,int keyup,int keydown,int keyjump,int keyshoot,int music_volume,int sound_volume)
 {
  FILE *fd;
...
void Game::layer_OSD(int alpha) {	OSD->drawOSD(0,0); }
  ...
  for(int j=0;j<N_BVASE;j+=2)
   {
    ...
    for(int i=0;i<N_BC;i++) if(!BVASE[j]->isActive() && !BVASE[j+1]->isActive()) BC[i]->attachVase(BVASE[j]); // Non serve per big_crab poich� e indistruttibile ...
...
void Game::collisionPlayerBonus()
 {
  // Player con bonus
...
void Game::collisionShildBullets()
 {
  // Shild con bullets
...
void Game::collisionPlayerEnemies()
 {
  // Player con enemies
...
void Game::collisionWeaponEnemies()
 {
  // Weapon con enemies
...
void Game::collisionMagicEnemies()
 {
  // Magic con enemies
...
void Game::collisionPlayerBullets()
 {
  // Player con bullets
  if(PL->isHittable() && !PL->isGold() && !PL->isMagicFire())
   {
...
void Game::debug(BITMAP *bmp, int alpha)
 {
