...
map::map(const char *bmp_filename,const char *map_filename, BITMAP *out,bool mainscroll,const char *plat_filename,const char *dat_plat_filename,const char *stairs_filename,const char *wall_filename,const char *dat_wall_filename,bool loadall)
 {
...
void map::loadMap(const char *bmp_filename,const char *map_filename)
 {
  int v,max;
  FILE *fd;
  BITMAP *aux;
...
void map::loadPlatform(const char *plat_filename,const char *bmp_plat_filename)
 {
  int v;
  FILE *fd;
...
void map::loadStairs(const char *stairs_filename)
 {
 	int v;
 	FILE *fd;
...
void map::loadWall(const char *wall_filename,const char *dat_wall_filename)
 {
 	int v;
 	FILE *fd;
...
void map::drawMap()
 {
     ...
     int pos_x = x+tilepos_x;
     int pos_y = y+tilepos_y;
     int tile_index = data[pos_x][pos_y];
     int scroll_to_x = x*tile_w-scroll_dx;
     int scroll_to_y = y*tile_h-scroll_dy;

      /*printf("pos_x=%d\n", pos_x);
      printf("pos_y=%d\n", pos_y);
      printf("tile_index=%d\n", tile_index);
      printf("scroll_to_x=%d\n", scroll_to_x);
      printf("scroll_to_y=%d\n", scroll_to_y);*/

     if (tile_index >= 0 && tile_index < 32*32) {
 	    draw_sprite(out,tile[tile_index],scroll_to_x,scroll_to_y);
     }
...
int map::getActPlatformMobile(int x,int y)
 {
  ...
  if(y/tile_h<0) return max_bound_y;
  for(int i=0;i<n_plat;i++) // Per tutte le piattaforme mobili (le pi� in alto devono avere indice basso)
   ...
   for(int y=0;y<=num_row;y++)
    {
	 ...
	 if(v>=0 && v<64) // Scala di mezzo/bassa (da 0 a 31, >= a 32 � come se fosse 31)
	  {
	   if(v>=32) v=31;
       for(int i=0;i<=v;i++)
        {
