...
mobile::mobile(const char *filename, const char *dat_filename, int sx, int sy, int fra, int end_f, int typ, BITMAP *out, int i, mobile *p)
 {
 	_i=i;
 	...
 	ti[1]=new timer_D(1); // Explode time
 	ti[2]=new timer_D(0);
 	ti[3]=new timer_D(0);
 	ti[4]=new timer_D(0);
 }
...
void mobile::loadMobile(const char *dat_mobile_filename)
 {
 	int v;
 	FILE *fd;
...
void mobile::loadSpriteFrames(const char *filename)
 {
  BITMAP *aux;
