...
fire_grizzly::fire_grizzly(const char *filename, int sx, int sy, BITMAP *bmp, int i, fire_grizzly *p) : sprite(filename,bmp,sx,sy,i,p)
 {
  min_x=0; max_x=0;
  ...
  ti[2]=new timer_D(5);  // Hitted time (stop time)
  ti[3]=new timer_D(random(100)+100); // Fire time
  ti[4]=new timer_D(1);  // Move time
 }
...
void fire_grizzly::loadSample(const char *filename, int n)
 {
...
void fire_grizzly::animSprite()
 {
  ...
  if(fire) { if(act_frame==7) { shootBullet(); fire=0; } }
   ...
   {
 	  if(*ctrl_x+48<pos_x && pos_x>min_x+100){side=1;}
    if(*ctrl_x-48>pos_x && pos_x<max_x-100){side=0;}
 	  if(pos_x<min_x) { side=0; } if(pos_x>max_x) { side=1; }
 	  ti[0]->setTick(4);
   }
 }
