...
firewall::firewall(const char *filename, int sx, int sy, int my, int co, BITMAP *bmp, int i, firewall *p) : sprite(filename,bmp,sx,sy,i,p)
 {
  maxy=my; coun=co; max_count=30;
  resetObj(true);
  ti[0]=new timer_D(16); // Restart time
  ti[1]=new timer_D(3);  // Anim time
  ti[2]=new timer_D(0);
  ti[3]=new timer_D(0);
  ti[4]=new timer_D(0);
 }
...
void firewall::loadSample(const char *filename, int n)
 {
