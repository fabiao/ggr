...
bat::bat(const char *filename, int sx, int sy, BITMAP *bmp, int i, bat *p) : sprite(filename,bmp,sx,sy,i,p)
 {
  resetObj(true);
  ti[0]=new timer_D(7); // Anim time
  ti[1]=new timer_D(1); // Explode time
  ti[2]=new timer_D(random(700)+100); // Restart time
  ti[3]=new timer_D(0);
  ti[4]=new timer_D(0);
 }
...
void bat::loadSample(const char *filename, int n)
 {
