...
little_devil::little_devil(const char *filename, int sx, int sy, BITMAP *bmp, int i, little_devil *p) : sprite(filename,bmp,sx,sy,i,p)
 {
  ...
  ti[1]=new timer_D(1); // Move time
  ti[2]=new timer_D(2); // Explode time
  ti[3]=new timer_D(10); // Shoot time
  ti[4]=new timer_D(100); // Change time
 }
...
void little_devil::loadSample(const char *filename, int n)
 {
