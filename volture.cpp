...
volture::volture(const char *filename, int sx, int sy, BITMAP *bmp, int i, volture *p) : sprite(filename,bmp,sx,sy,i,p)
 {
  resetObj(true);
  ti[0]=new timer_D(4);  // Anim time
  ti[1]=new timer_D(2);  // Explode time
  ti[2]=new timer_D(5);  // Stop time
  ti[3]=new timer_D(10); // Fly anim time
  ti[4]=new timer_D(0);
 }
...
void volture::loadSample(const char *filename, int n)
 {
