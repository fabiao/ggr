...
player::player(const char *filename, int rsx, int rsy, BITMAP *bmp, int i, player *p) : sprite(filename,bmp,rsx,rsy,i,p)
 {
  BITMAP *aux;
  char str[255];
  ...
  ti[0]=new timer_D(2);   // Move time
  ti[1]=new timer_D(2);   // Fire time
  ti[2]=new timer_D(3);   // Flashing time
  ti[3]=new timer_D(130); // Invisible time
  ti[4]=new timer_D(4);   // Dead/Gold time
  ti[5]=new timer_D(16);  // Gold time
  ti[6]=new timer_D(70);  // Magicfire time (cambiare il tick a secondo dell'arma)
  ti[7]=new timer_D(1);   // Gauge anim time
 }
...
void player::loadSample(const char *filename, int n)
 {
...
void player::drawSprite()
 {
  ...
  if(Y<=-5) { glob_y+=2; } // Risolve bug uscita da schermo in alto e non c'� scroll verticale
...
void player::calcMax()
 {
  // Calcola i limiti entro i quali il giocatore pu� muoversi
