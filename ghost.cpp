...
ghost::ghost(const char *filename, int sx, int sy, BITMAP *bmp, int i, ghost *p) : sprite(filename,bmp,sx,sy,i,p)
 {
  resetObj(true);
  ti[0]=new timer_D(10);  // Anim time
  ti[1]=new timer_D(2);   // Explode time
  ti[2]=new timer_D(random(100)+50); // Far time
  ti[3]=new timer_D(random(200)+150); // Move time
  ti[4]=new timer_D(0);
 }
...
void ghost::loadSample(const char *filename, int n)
 {
