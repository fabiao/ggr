...
skeleton::skeleton(const char *filename, int sx, int sy, BITMAP *bmp, int i, skeleton *p) : sprite(filename,bmp,sx,sy,i,p)
 {
  resetObj(true);
  ti[0]=new timer_D(3); // Move time
  ti[1]=new timer_D(8); // Rise/Unrise time
  ti[2]=new timer_D(1); // Explode time
  ti[3]=new timer_D(1); // Speed time
  ti[4]=new timer_D(0);
 }
...
void skeleton::loadSample(const char *filename, int n)
 {
