...
bullet::bullet(const char *filename, BITMAP *out, int i, int sta_fr,int end_fr,int ty)
 {
  _i=i;
  start_frame=sta_fr;
  end_frame=end_fr;
  ...
  ti[1]=new timer_D(1); // Rotating time (aumento del raggio di rotazione)
  ti[2]=new timer_D(0);
  ti[3]=new timer_D(0);
  ti[4]=new timer_D(0);
 }
...
void bullet::loadSample(const char *filename, int n)
 {
  ...
  if(fire)
   {
	...
	if(act_frame>end_frame){act_frame=start_frame;}
