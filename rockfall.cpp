...
rockfall::rockfall(const char *filename, int sx, int sy, int my, int co, BITMAP *bmp, int fr, int i, rockfall *p) : sprite(filename,bmp,sx,sy,i,p)
 {
...
void rockfall::loadSample(const char *filename, int n)
 {
  ...
  if(action==_FALL)
   {
  	...
  	if(count%4==0) if(dy<9) dy++; // Gravit�
  	count++; pos_y+=dy;
   }
