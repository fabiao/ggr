...
red_arremer_king::red_arremer_king(const char *filename, int sx, int sy, BITMAP *bmp, int i, red_arremer_king *p) : sprite(filename,bmp,sx,sy,i,p)
 {
  resetObj(true);
  ti[0]=new timer_D(3);   // Anim time
  ti[1]=new timer_D(280+random(80)); // Attack time
  ti[2]=new timer_D(400+random(150)); // Move time
  ti[3]=new timer_D(2);   // Explode time
  ti[4]=new timer_D(3);   // Stop time
 }
...
void red_arremer_king::loadSample(const char *filename, int n)
 {
