...
magic::magic(const char *filename, BITMAP *out, int i, magic *p)
 {
  ...
  ti[1]=new timer_D(2); // Anim time
  ti[2]=new timer_D(1); // Move time
  ti[3]=new timer_D(0);
  ti[4]=new timer_D(0);
 }
...
void magic::loadSpriteFrames(const char *filename)
 {
  BITMAP *aux;
...
void magic::loadSample(const char *filename, int n)
 {
